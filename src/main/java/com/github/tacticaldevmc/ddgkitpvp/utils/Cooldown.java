...
public class Cooldown {
    ...
    public static void stopPlayers(String cooldownName) {
        for(Map.Entry<String, Cooldown> entry : cooldowns.entrySet()) {
            if(entry.getKey().contains(cooldownName)) {
                stop(UUID.fromString(entry.getKey().split(cooldownName)[0]), cooldownName);
            }
        }
    }
